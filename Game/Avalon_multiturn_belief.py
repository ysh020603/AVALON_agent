...
from prompts.Avalon_belief_prompts import (
    MAIN_CHANNEL_SYSTEM_ADDENDUM,
    get_avalon_belief_system_prompt,
)
...
class Game_Avalon_MultiturnWithBelief(Game_Avalon_Multiturn):
    ...
    def __init__(self, agents, log_tag: str = ""):
        ...
        self._patch_main_system_for_belief_agents()
        self._init_belief_system_prompts()

    def _patch_main_system_for_belief_agents(self) -> None:
        for agent in self.agents:
            if not isinstance(agent, BeliefAgent):
                continue
            pid = agent.player_id
            content = self.agent_memories[pid][0]["content"]
            self.agent_memories[pid][0]["content"] = content + "\n" + MAIN_CHANNEL_SYSTEM_ADDENDUM
            key = f"player_{pid}"
            if key in self.game_log.get("agents_info", {}):
                self.game_log["agents_info"][key]["system_prompt"] = self.agent_memories[pid][0]["content"]
    ...
    def call_agent(self, pid, phase, context=None) -> Tuple[str, List]:
        ...
        buffered_obs = list(self.pending_observations[pid])
        response, messages = super().call_agent(pid, phase, context)
        agent = self.get_agent(pid)
        if isinstance(agent, BeliefAgent):
            belief_raw = agent.infer_identities(phase, buffered_obs, context)
